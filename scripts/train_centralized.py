...
def parse_args():
    ...
    parser.add_argument(
        "--experiment-name", type=str, default="centralized", help="Experiment name"
    )
    parser.add_argument(
        "--checkpoint-dir",
        type=str,
...
def main():
    ...
    for run in range(args.num_runs):
        ...
        experiment_name = (
            f"{args.experiment_name}_run{run}"
            if args.num_runs > 1
        ...
        model = create_dino_vit(
            num_classes=100, freeze_backbone=args.freeze_backbone, device=args.device
        )

        param_counts = model.count_parameters()
        logger.info(
            f"model params: Total={param_counts['total']:,}, Trainable={param_counts['trainable']:,}"
        )
        ...
        if args.hp_search:
            ...
            hp_results = trainer.hyperparameter_search(
                lr_range=[1e-4, 5e-4, 1e-3, 5e-3, 1e-2], epochs_per_trial=args.hp_epochs
            )
