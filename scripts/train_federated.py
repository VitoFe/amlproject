...
def parse_args():
    ...
    parser.add_argument(
        "--experiment-name", type=str, default="federated", help="Experiment name"
    )
    parser.add_argument(
        "--checkpoint-dir",
        type=str,
...
def run_single_experiment(args, nc=None, local_steps=None, run=0):
    ...
    config = {
        ...
        "seed": run_seed,
        "sharding": {"strategy": args.sharding, "nc": nc or args.nc},
    }

    sharding_str = "iid" if args.sharding == "iid" else f"noniid_nc{nc or args.nc}"
    experiment_name = (
        f"{args.experiment_name}_{sharding_str}_J{local_steps or args.local_steps}"
    )
    if args.num_runs > 1:
    ...
    test_metrics = trainer.train(
        resume=True, save_every=args.save_every, use_wandb=args.use_wandb
    )
