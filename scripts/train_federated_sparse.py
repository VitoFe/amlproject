...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Federated Sparse Fine-tuning on CIFAR-100"
    )

    parser.add_argument(
        "--num-clients", type=int, default=100, help="Total number of clients (K)"
    )
    parser.add_argument(
        "--participation-rate",
        type=float,
        default=0.1,
        help="Client participation rate (C)",
    )
    parser.add_argument(
        "--local-steps", type=int, default=4, help="Local training steps per client (J)"
    )
    parser.add_argument(
        "--num-rounds", type=int, default=500, help="Communication rounds"
    )
    ...
    parser.add_argument(
        ...
        default="iid",
        choices=["iid", "non_iid"],
        help="Data sharding strategy",
    )
    parser.add_argument(
        "--nc", type=int, default=10, help="Classes per client for non-IID"
    )
    ...
    parser.add_argument(
        "--fisher-samples", type=int, default=512, help="Samples for Fisher computation"
    )
    ...
    parser.add_argument(
        "--save-every", type=int, default=50, help="Save checkpoint every N rounds"
    )
    ...
    parser.add_argument(
        "--wandb-project", type=str, default="federated-task-arithmetic"
    )

    parser.add_argument("--config", type=str, help="Path to YAML config file")

    parser.add_argument(
        "--compare-strategies", action="store_true", help="Compare all mask strategies"
    )
    parser.add_argument(
        "--sparsity-experiment",
        action="store_true",
...
def run_single_experiment(args, sparsity_ratio=None, mask_strategy=None, run=0):
    ...
    config = {
        ...
        "seed": run_seed,
        "sharding": {"strategy": args.sharding, "nc": args.nc},
        "sparse": {
    ...
    train_dataset, val_dataset, test_dataset = get_cifar100_datasets(
        data_dir=args.data_dir, val_split=args.val_split, seed=run_seed
    )
    ...
    model = create_dino_vit(num_classes=100, freeze_backbone=False, device=args.device)

    param_counts = model.count_parameters()
    logger.info(
        f"Model parameters: Total={param_counts['total']:,}, Trainable={param_counts['trainable']:,}"
    )
...
def main():
    ...
    if args.compare_strategies:
        ...
        for strategy in strategies:
            ...
            for run in range(args.num_runs):
                metrics, best_val = run_single_experiment(
                    args, mask_strategy=strategy, run=run
                )
        ...
        print("-" * 45)
        for strategy, r in sorted(
            results.items(), key=lambda x: x[1]["test_accuracy"], reverse=True
        ):
            print(
                f"{strategy:<25} {r['test_accuracy']:.4f} ± {r['test_accuracy_std']:.4f}"
            )
    ...
    if args.sparsity_experiment:
        ...
        for sparsity in args.sparsity_values:
            ...
            for run in range(args.num_runs):
                metrics, _ = run_single_experiment(
                    args, sparsity_ratio=sparsity, run=run
                )
                run_results.append(metrics["accuracy"])
        ...
        for sparsity, r in results.items():
            print(
                f"{sparsity:<15} {r['test_accuracy']:.4f} ± {r['test_accuracy_std']:.4f}"
            )
