...
def parse_args():
    ...
    parser.add_argument(
        "--quick", action="store_true", help="Quick test run with reduced parameters"
    )
    parser.add_argument(
        "--full", action="store_true", help="Full experiments (long runtime)"
    )
    parser.add_argument(
...
def run_heterogeneity_experiment(args, params, train_dataset, val_loader, test_loader):
    ...
    for nc in params["nc_values"]:
        ...
        logger.info(
            f"Nc={nc}: {results[nc]['mean_accuracy']:.4f} ± {results[nc]['std_accuracy']:.4f}"
        )
...
def run_local_steps_experiment(args, params, train_dataset, val_loader, test_loader):
    ...
    for j in params["j_values"]:
        ...
        logger.info(
            f"J={j}: {results[j]['mean_accuracy']:.4f} ± {results[j]['std_accuracy']:.4f}"
        )
...
def run_sparse_experiment(args, params, train_dataset, val_loader, test_loader):
    ...
    for sparsity in params["sparsity_values"]:
        ...
        logger.info(
            f"Sparsity={sparsity}: {results[sparsity]['mean_accuracy']:.4f} ± {results[sparsity]['std_accuracy']:.4f}"
        )
...
def run_mask_strategy_experiment(args, params, train_dataset, val_loader, test_loader):
    ...
    for strategy in strategies:
        ...
        logger.info(
            f"{strategy}: {results[strategy]['mean_accuracy']:.4f} ± {results[strategy]['std_accuracy']:.4f}"
        )
...
def main():
    ...
    # Setup logging
    logger = setup_logging(log_dir=args.log_dir, experiment_name="all_experiments")

    logger.info(
        f"Starting experiments with {'quick' if args.quick else 'full'} parameters"
    )
    ...
    train_dataset, val_dataset, test_dataset = get_cifar100_datasets(
        data_dir=args.data_dir, val_split=0.1, seed=args.seed
    )
    ...
    # Run experiments
    run_all = not (
        args.only_centralized
        or args.only_federated
        or args.only_sparse
        or args.only_extension
    )
    ...
    if "centralized" in all_results:
        ...
        print(
            f"Centralized Baseline: {r['mean_accuracy']:.4f} ± {r['std_accuracy']:.4f}"
        )
    ...
    if "mask_strategies" in all_results:
        ...
        for strategy, r in sorted(
            all_results["mask_strategies"].items(),
