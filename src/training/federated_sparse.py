"""
Federated Sparse Fine-tuning Trainer

Combines Federated Learning with Task Arithmetic techniques:
sparse fine-tuning using gradient masks based on parameter sensitivity.

Reference: Iurada et al., "Efficient Model Editing with Task-Localized 
Sparse Fine-tuning". ICLR 2025.
"""

import torch
import torch.nn as nn
from torch.utils.data import DataLoader
from typing import Dict, Optional, List, Any
from copy import deepcopy

from .federated import FederatedTrainer
from ..optimizers.sparse_sgdm import create_sparse_optimizer
from ..optimizers.fisher import (
    calibrate_gradient_mask,
    MaskStrategy,
    get_mask_sparsity
)
from ..models.dino_vit import get_model_state_dict
from ..utils.logging import log_metrics


class FederatedSparseTrainer(FederatedTrainer):
    """
    Federated Learning with Sparse Fine-tuning.
    
    Extends FedAvg with gradient masking based on parameter sensitivity,
    allowing clients to update only a subset of parameters chosen to
    minimize interference during aggregation.
    
    Key additions:
    - Gradient mask calibration using Fisher Information
    - SparseSGDM optimizer for masked updates
    - Multiple mask strategies (least/most sensitive, magnitude, random)
    """
    
    def __init__(
        self,
        model: nn.Module,
        train_dataset,
        val_loader: DataLoader,
        test_loader: DataLoader,
        config: Dict[str, Any],
        device: str = 'cuda',
        experiment_name: str = 'federated_sparse'
    ):
        """
        Initialize federated sparse trainer.
        
        Args:
            model: PyTorch model
            train_dataset: Full training dataset
            val_loader: Validation data loader
            test_loader: Test data loader
            config: Configuration with additional keys:
                - sparse.sparsity_ratio: Fraction of params to freeze
                - sparse.calibration_rounds: Rounds for Fisher averaging
                - sparse.mask_strategy: Mask selection strategy
                - sparse.fisher_samples: Samples for Fisher computation
            device: Device for computation
            experiment_name: Name for logging
        """
        super().__init__(
            model, train_dataset, val_loader, test_loader,
            config, device, experiment_name
        )
        
        # Sparse fine-tuning parameters
        sparse_config = config.get('sparse', {})
        self.sparsity_ratio = sparse_config.get('sparsity_ratio', 0.9)
        self.calibration_rounds = sparse_config.get('calibration_rounds', 5)
        self.fisher_samples = sparse_config.get('fisher_samples', 512)
        
        # Parse mask strategy
        strategy_str = sparse_config.get('mask_strategy', 'least_sensitive')
        self.mask_strategy = MaskStrategy(strategy_str)
        
        # Gradient masks (calibrated before training)
        self.gradient_masks: Optional[Dict[str, torch.Tensor]] = None
        
        self.logger.info(f"Sparse Fine-tuning Setup:")
        self.logger.info(f"  - Sparsity ratio: {self.sparsity_ratio}")
        self.logger.info(f"  - Calibration rounds: {self.calibration_rounds}")
        self.logger.info(f"  - Mask strategy: {self.mask_strategy.value}")
        self.logger.info(f"  - Fisher samples: {self.fisher_samples}")
    
    def calibrate_masks(
        self,
        calibration_loader: Optional[DataLoader] = None
    ) -> Dict[str, torch.Tensor]:
        """
        Calibrate gradient masks using specified strategy.
        
        For sensitivity-based strategies, computes Fisher Information
        over multiple rounds and averages. For magnitude-based, uses
        current weight magnitudes.
        
        Args:
            calibration_loader: DataLoader for Fisher computation
                               (uses subset of training data if None)
        
        Returns:
            Dict mapping parameter name to binary mask
        """
        self.logger.info(f"Calibrating gradient masks with strategy: {self.mask_strategy.value}")
        
        # Use validation loader for calibration if not specified
        if calibration_loader is None:
            calibration_loader = self.val_loader
        
        self.gradient_masks = calibrate_gradient_mask(
            self.model,
            calibration_loader,
            self.criterion,
            sparsity_ratio=self.sparsity_ratio,
            strategy=self.mask_strategy,
            num_calibration_rounds=self.calibration_rounds,
            device=self.device,
            num_fisher_samples=self.fisher_samples,
            show_progress=True
        )
        
        # Log sparsity statistics
        sparsity_stats = get_mask_sparsity(self.gradient_masks)
        self.logger.info(f"Mask calibration complete:")
        self.logger.info(f"  - Global sparsity: {sparsity_stats['global_sparsity']:.4f}")
        self.logger.info(f"  - Trainable params: {sparsity_stats['trainable_params']:,}")
        self.logger.info(f"  - Masked params: {sparsity_stats['masked_params']:,}")
        
        return self.gradient_masks
    
    def _client_update(
        self,
        client_id: int,
        global_state: Dict[str, torch.Tensor]
    ) -> Dict[str, torch.Tensor]:
        """
        Perform sparse local training for a single client.
        
        Uses SparseSGDM optimizer with gradient masks to update
        only the selected subset of parameters.
        
        Args:
            client_id: Client identifier
            global_state: Global model state dict
        
        Returns:
            Updated local model state dict
        """
        # Ensure masks are calibrated
        if self.gradient_masks is None:
            raise RuntimeError("Gradient masks not calibrated. Call calibrate_masks() first.")
        
        # Load global model
        self.model.load_state_dict(global_state)
        self.model.train()
        
        # Create sparse optimizer
        optimizer = create_sparse_optimizer(
            self.model,
            lr=self.client_lr,
            momentum=self.momentum,
            weight_decay=self.weight_decay,
            gradient_masks=self.gradient_masks
        )
        
        # Get client data
        client_loader = self._get_client_dataloader(client_id)
        
        # Perform local steps with sparse updates
        step = 0
        while step < self.local_steps:
            for inputs, targets in client_loader:
                if step >= self.local_steps:
                    break
                
                inputs, targets = inputs.to(self.device), targets.to(self.device)
                
                optimizer.zero_grad()
                outputs = self.model(inputs)
                loss = self.criterion(outputs, targets)
                loss.backward()
                optimizer.step()  # Applies gradient mask internally
                
                step += 1
        
        return get_model_state_dict(self.model)
    
    def train(
        self,
        resume: bool = True,
        save_every: int = 50,
        use_wandb: bool = False,
        calibrate_masks: bool = True
    ) -> Dict[str, float]:
        """
        Train using Federated Sparse Fine-tuning.
        
        Optionally calibrates gradient masks before training,
        then performs FedAvg with sparse local updates.
        
        Args:
            resume: Whether to try resuming
            save_every: Save checkpoint every N rounds
            use_wandb: Whether to log to W&B
            calibrate_masks: Whether to calibrate masks before training
        
        Returns:
            Final test metrics
        """
        # Calibrate gradient masks
        if calibrate_masks and self.gradient_masks is None:
            self.calibrate_masks()
        
        # Use parent class training loop
        return super().train(
            resume=resume,
            save_every=save_every,
            use_wandb=use_wandb
        )
    
    def compare_mask_strategies(
        self,
        strategies: List[str] = None,
        num_rounds: int = 100
    ) -> Dict[str, Dict[str, float]]:
        """
        Compare different gradient mask strategies.
        
        Runs abbreviated training with each strategy and reports
        final validation accuracy.
        
        Args:
            strategies: List of strategy names to compare
            num_rounds: Rounds per strategy
        
        Returns:
            Dict mapping strategy name to metrics
        """
        if strategies is None:
            strategies = [
                'least_sensitive',
                'most_sensitive', 
                'lowest_magnitude',
                'highest_magnitude',
                'random'
            ]
        
        self.logger.info(f"Comparing mask strategies: {strategies}")
        
        # Store original state
        original_state = get_model_state_dict(self.model)
        original_num_rounds = self.num_rounds
        self.num_rounds = num_rounds
        
        results = {}
        
        for strategy in strategies:
            self.logger.info(f"\n{'='*50}")
            self.logger.info(f"Testing strategy: {strategy}")
            self.logger.info(f"{'='*50}")
            
            # Reset model
            self.model.load_state_dict(original_state)
            self.best_val_accuracy = 0.0
            self.current_epoch = 0
            
            # Update strategy
            self.mask_strategy = MaskStrategy(strategy)
            self.gradient_masks = None  # Force recalibration
            
            # Calibrate new masks
            self.calibrate_masks()
            
            # Clear metrics history
            for key in self.metrics_history:
                self.metrics_history[key] = []
            
            # Train
            test_metrics = super().train(resume=False, save_every=num_rounds+1)
            
            results[strategy] = {
                'test_accuracy': test_metrics['accuracy'],
                'test_loss': test_metrics['loss'],
                'best_val_accuracy': self.best_val_accuracy
            }
            
            self.logger.info(f"Strategy {strategy}: Test Acc = {test_metrics['accuracy']:.4f}")
        
        # Restore
        self.num_rounds = original_num_rounds
        self.model.load_state_dict(original_state)
        
        return results


def run_all_mask_strategies_experiment(
    model_fn,
    train_dataset,
    val_loader: DataLoader,
    test_loader: DataLoader,
    base_config: Dict[str, Any],
    device: str = 'cuda'
) -> Dict[str, Dict[str, float]]:
    """
    Convenience function to run experiments with all mask strategies.
    
    Args:
        model_fn: Function that returns a fresh model
        train_dataset: Training dataset
        val_loader: Validation loader
        test_loader: Test loader
        base_config: Base configuration
        device: Device
    
    Returns:
        Results for each strategy
    """
    strategies = [
        'least_sensitive',
        'most_sensitive',
        'lowest_magnitude', 
        'highest_magnitude',
        'random'
    ]
    
    results = {}
    
    for strategy in strategies:
        # Create fresh model
        model = model_fn()
        
        # Update config
        config = deepcopy(base_config)
        config['sparse']['mask_strategy'] = strategy
        
        # Create trainer
        trainer = FederatedSparseTrainer(
            model=model,
            train_dataset=train_dataset,
            val_loader=val_loader,
            test_loader=test_loader,
            config=config,
            device=device,
            experiment_name=f'federated_sparse_{strategy}'
        )
        
        # Train
        test_metrics = trainer.train(resume=False)
        
        results[strategy] = {
            'test_accuracy': test_metrics['accuracy'],
            'test_loss': test_metrics['loss'],
            'best_val_accuracy': trainer.best_val_accuracy
        }
    
    return results
