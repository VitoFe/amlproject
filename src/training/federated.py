...
from ..data.dataset import get_client_dataloader
from ..data.sharding import create_client_splits, ShardingStrategy, get_sharding_stats
from ..utils.logging import log_metrics
from ..utils.early_stopping import EarlyStopping


def _get_trainable_state(model: nn.Module) -> Dict[str, torch.Tensor]:
    """Get only trainable parameters (faster than full state_dict)."""
    return {
        name: p.data.clone() for name, p in model.named_parameters() if p.requires_grad
    }


def _set_trainable_state(model: nn.Module, state: Dict[str, torch.Tensor]) -> None:
    """Set only trainable parameters from state dict."""
    with torch.no_grad():
...
class FederatedTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        super().__init__(
            model, temp_loader, val_loader, test_loader, config, device, experiment_name
        )
        ...
        self.eval_every = config.get(
            "eval_every", 10
        )  # Evaluate every N rounds (10 = skip 9 rounds for speed)
        ...
        if self.early_stopping_patience > 0:
            ...
            min_delta = (
                0.0001 if self.sharding_strategy == ShardingStrategy.NON_IID else 0.001
            )
        ...
        self.logger.info(
            f"  - Learning rate: {self.base_lr} (scheduler: {self.scheduler_type})"
        )
        ...
        if self.early_stopping:
            self.logger.info(
                f"  - Early stopping patience: {self.early_stopping_patience} rounds"
            )
    ...
    def _create_client_splits(self):
        ...
        if hasattr(self.train_dataset, "targets"):
            ...
            labels = np.array(self.train_dataset.dataset.targets)[
        ...
        self.logger.info(
            f"  - Classes per client: {stats['classes_per_client_mean']:.1f} ± {stats['classes_per_client_std']:.1f}"
        )
    ...
    def _aggregate(
        ...
        Args:
            client_states: List of client TRAINABLE state dicts
            client_weights: Optional weights for averaging

        Returns:
            Aggregated global trainable state dict
        """
        ...
        # Initialize with first client scaled by its weight (avoid extra zero tensor)
        aggregated = {
            key: weights[0] * val.clone() for key, val in client_states[0].items()
        }

        # Add remaining clients
        for state, weight in zip(client_states[1:], weights[1:]):
            for key in aggregated:
                aggregated[key].add_(state[key], alpha=weight)
    ...
    def train(
        ...
        # Only track trainable params (much smaller than full state_dict!)
        global_state = _get_trainable_state(self.model)
        ...
        for round_idx in range(start_round, self.num_rounds):
            ...
            should_eval = (
                ...
                or (round_idx == self.num_rounds - 1)
                or (round_idx == 0)
            )
            ...
            if should_eval:
                ...
                # Save checkpoint
                if (round_idx + 1) % save_every == 0 or val_metrics[
                    "accuracy"
                ...
                if val_metrics["accuracy"] > self.best_val_accuracy:
                    ...
                    self.logger.info(
                        f"Round {round_idx}: New best val accuracy: {self.best_val_accuracy:.4f}"
                    )
                ...
                if self.early_stopping is not None:
                    if self.early_stopping(val_metrics["accuracy"], round_idx):
                        self.logger.info(
                            f"Early stopping triggered at round {round_idx}!"
                        )
                        self.logger.info(
                            f"Best validation accuracy was {self.early_stopping.best_score:.4f} at round {self.early_stopping.best_epoch}"
                        )
                        break
